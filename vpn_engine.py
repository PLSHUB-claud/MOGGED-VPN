...
class VpnEngine:
    ...
    def prepare_config(self, ovpn_base64: str, mode: str='full') -> str:
        ...
        for line in raw_text.splitlines():
            ...
            if mode == 'discord' and (clean.startswith('redirect-gateway') or clean.startswith('route-gateway')):
                continue
            lines.append(line)
        custom_directives = ['', '# --- MOGGED VPN CONNECTION OPTIONS ---', 'nobind', 'persist-key', 'persist-tun', 'verb 3', 'resolv-retry 2', 'connect-timeout 7', 'hand-window 8', 'server-poll-timeout 5', 'mssfix 1360', 'sndbuf 524288', 'rcvbuf 524288', 'windows-driver wintun']
        if mode == 'full':
        ...
        config_path = os.path.join(temp_dir, f'mogged_vpn_{int(time.time())}.ovpn')
                ...
                if line is not None:
                    ...
                    if line_str:
                        ...
                        if 'Initialization Sequence Completed' in line_str:
                            ...
                            break
                        if any((err in line_str for err in ['AUTH_FAILED', 'TLS Error', 'Cannot resolve host', 'Connection refused', 'Connection timed out', 'SIGTERM', 'process exiting'])):
                            logger.warning(f'Server error during handshake: {line_str[:80]}')
                            break
    ...
    def _kill_process(self):
        if self.process:
            try:
                subprocess.run(['taskkill', '/F', '/T', '/PID', str(self.process.pid)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0)
            except Exception:
                try:
            ...
            self.process = None
        try:
            subprocess.run(['taskkill', '/F', '/IM', 'openvpn.exe'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0)
        except Exception:
            pass
    ...
    def disconnect(self):
        ...
        self._update_status(STATUS_CONNECTING, 'Disconnecting...')
        self._kill_process()
        try:
